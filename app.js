...
function renderCartPageTab() {
  const body = $("cartPageItems"), foot = $("cartPageFooter");
  ...
  cart.forEach(i => {
    ...
    el.querySelector(".trash").onclick = () => { removeFromCart(i.product.id); renderCartPageTab(); }; body.appendChild(el);
  });
...
function renderProfile() {
  ...
  if(user) {
     ...
     displayObj.textContent = "Guest Access"; nameObj.textContent = "Welcome Guest";
...
if($("editProfileBtn")) {
  $("editProfileBtn").onclick = async () => {
     ...
     if(newName !== null && newName.trim() !== "") {
        const btn = $("editProfileBtn"); const originalHtml = btn.innerHTML; 
  ...
  $("profilePicInput").onchange = function(e) {
     ...
     reader.onload = function(event) {
        ...
        img.onload = function() {
            ...
            let width = img.width, height = img.height; const MAX_SIZE = 250; 
            ...
            try {
                ...
                $("profileImg").src = dataUrl; $("profileImg").style.opacity = "1"; 
                const user = window.fbAuth ? window.fbAuth.currentUser : null;
...
$("logoBtn").onclick = () => { if (mainCategories.length > 0) selectMainCat(mainCategories[0].id); };
...
function renderMainCats() {
  ...
  mainCategories.forEach((cat, i) => {
    ...
    btn.innerHTML = `<span class="mc-label">${cat.name}</span>`; btn.onclick = () => selectMainCat(cat.id);
...
function openProductDetail(p) {
  lockScroll(); currentDetailProduct = p; const price = finalPrice(p), inStock = p.inStock !== false, cat = getCat(p.mainCategoryId);
  const slider = $("pdImageSlider"), dotsWrap = $("pdImageDots"); slider.innerHTML = ""; dotsWrap.innerHTML = "";
  let images = Array.isArray(p.image) ? p.image : [p.image]; if(images.length === 0) images = ["placeholder.jpg"];
  images.forEach((imgUrl, i) => {
    const imgEl = document.createElement("img"); imgEl.src = imgUrl; imgEl.onclick = () => { $("fullImage").src = imgUrl; $("imageViewer").classList.remove("hidden"); allowZoom(); };
    slider.appendChild(imgEl); if(images.length > 1) { const dot = document.createElement("div"); dot.className = "dot" + (i === 0 ? " active" : ""); dotsWrap.appendChild(dot); }
  });
  ...
  if (inStock) {
    ...
    addBtn.onclick = () => { addToCart(p); renderCartCount(); alert("Added to cart!"); }; buyBtn.onclick = () => directBuyCheckout(p);
...
function syncDetailCartBadge() { renderCartCount(); }
function buildHorizSection(title, list) {
  ...
  list.forEach(p => {
    const price = finalPrice(p), inStock = p.inStock !== false, mainImg = (Array.isArray(p.image) && p.image.length > 0) ? p.image[0] : "placeholder.jpg";
...
function tryUnlock() {
  if ($("pinInput").value === ADMIN_PIN) { $("adminPin").classList.add("hidden"); openAdmin(); } else { $("pinError").classList.remove("hidden"); }
}
...
function renderCatMgmt() {
  ...
  mainCategories.forEach(cat => {
    ...
    card.innerHTML = `<div class="cat-mgmt-head"><span>${cat.name}</span><div><button class="edit-cat-btn">Edit</button></div></div>`;
    list.appendChild(card);
  });
}
function syncAddProductDropdowns() { const pMainCat = $("pMainCat"); pMainCat.innerHTML = ""; mainCategories.forEach(cat => { const o = document.createElement("option"); o.value = cat.id; o.textContent = cat.name; pMainCat.appendChild(o); }); }

// SHIPROCKET LOGIC INTEGRATION IN ADMIN CARD VIEW LIST
window.renderAdminOrders = function(orders) {
  const list = $("adminOrdersList"); if (!list) return;
  list.innerHTML = "";
  orders.forEach(o => {
    ...
    div.innerHTML = `
      ...
      <div style="font-size:12px; color:var(--muted2); margin:5px 0;">Address: ${o.address}</div>
    ...
    div.querySelector(".save-track-btn").onclick = () => {
      ...
      if(window.updateOrderTrackingInFirebase) window.updateOrderTrackingInFirebase(o.id, linkVal);
    };
    list.appendChild(div);
  });
};

function renderAdminAdminProducts() { renderAdminProducts(); }
function openEditModal(p) { editingProductId = p.id; $("editModal").classList.remove("hidden"); }
